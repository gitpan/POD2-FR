...
paires suivantes sont équivalentes :
...
Attention : en dehors d'un test C<while> cela ne marche pas.
...
L<perlfunc/sort>. À cause de cette particularité $a et $b n'ont
pas besoin d'être déclaré (en utilisant local(), use vars, ou our())
même si vous utilisez le pragma vars strict. N'en faites pas des
variables lexicales avec C<my $a> ou C<my $b> si vous voulez avoir
la possibilité de les utiliser dans le bloc ou la fonction de
comparaison de sort().
...
Contient le groupement inclus dans les parenthèses correspondantes du dernier
motif trouvé, sans prendre en compte les motifs trouvés dans les sous-blocs
dont on est déjà sortis.(Mnémonique : comme \<I<chiffres>>.)  Ces variables
sont en lecture seule et ont une portée dynamique étendue au BLOC courant.
...
dans le BLOC courant). (Mnémonique : comme C<&> dans certains éditeurs de
...
La chaîne de caractères qui précède tout ce qui a été trouvé au cours de la
dernière recherche de motif réussie (non comptées les correspondances cachées
dans un BLOC ou un C<eval()> du BLOC courant).  (Mnémonique : C<`> est souvent
utilisé, en anglais, comme guillemet ouvrant dans les citations).  Variable en
lecture seule.
...
La chaîne de caractères qui suit tout ce qui a été trouvé au cours de la
dernière recherche de motif réussie (non comptées les correspondances cachées
dans un BLOC ou un eval() du BLOC courant). (Mnémonique : C<'> est souvent
utilisé, en anglais, comme guillemet fermant dans les citations).  Exemple :
...
Le texte du groupement parenthésé le plus récemment fermé (c.-à-d. le groupe
avec la parenthèse fermée la plus à droite) du modèle de la dernière recherche
de motifs réussie.
(Mnémonique: la parenthèse eNchâssée (éventuelle) qui a été plus récemment a fermé.)
...
multilignes, à 0 (ou undef) pour signifier à Perl que la chaîne ne contient
qu'une seule ligne, dans le but d'optimiser la recherche. La recherche de
motif sur des chaînes contenant plusieurs retours lignes peut produire des
résultats étranges quand "C<$*>" est à 0 ou undef. La valeur par défaut est
undef.  (Mnémonique : * remplace plusieurs.)  Cette variable n'influence que
l'interprétation de "C<^>" et de "C<$>".  Un retour ligne peut être recherché
même si C<$* == 0>.
...
(Mnémonique : beaucoup de programmes utilisent "." pour
...
Le séparateur d'enregistrement en lecture, par défaut retour-ligne.  Il
influence l'idée que Perl se fait d'une "ligne".  Fonctionne comme la variable
RS de B<awk>, y compris le traitement des lignes vides comme des délimiteurs
si initialisé à vide.  (Note : une ligne vide ne peut contenir ni espace, ni
tabulation.)  Vous pouvez le définir comme une chaîne de caractères pour
correspondre à un délimiteur multicaractère, ou à C<undef> pour lire jusqu'à
la fin du fichier. Notez que le fixer à C<"\n\n"> est légèrement différent que
de le fixer à C<""> si le fichier contient plusieurs lignes vides
consécutives. Le positionner à C<""> traitera deux lignes vides consécutives
(ou plus) comme une seule. Le positionner à C<"\n\n"> implique que le prochain
caractère lu appartient systématiquement à un nouveau paragraphe, même si
c'est un retour-ligne.  (Mnémonique: / est utilisé comme séparateur de ligne
quand on cite une poésie.)

    local $/;              # activer le mode "slurp"
    local $_ = <FH>;       # Fichier complet depuis la position courante
    s/\n[ \t]+/ /g;

Attention : La valeur de C<$/> doit être du texte et non une expression
...
sur les buffers d'entrée. Voir L<perlfunc/getc> pour cela.  (Mnémonique :
...
Le séparateur de champs pour l'opérateur print. Normalement l'opérateur print
sépare par des virgules les champs que vous avez spécifiés.  Pour obtenir un
comportement plus proche de B<awk>, initialisez cette variable comme vous le
feriez de la variable OFS de B<awk> pour spécifier ce qui doit être imprimé
entre chaque champ.  (Mnémonique : Ce qui est imprimé quand vous avez une
``,'' dans vos champs)
...
Le séparateur d'enregistrements pour l'opérateur print. Normalement,
l'opérateur print génère simplement les arguments tels que vous les avez
spécifiés, sans ajouter ni retour-ligne ni séparateur d'enregistrement.  Pour
obtenir un comportement plus proche de celui de B<awk>, initialisez cette
variable comme vous le feriez de la variable ORS de B<awk> pour préciser ce
qui doit être sorti en fin de traitement.  (Mnémonique : positionnez "C<$\>"
au lieu d'ajouter \n à la fin de chaque impression.  Ou : Comme C<$/>, mais
c'est ce que vous "obtenez" de Perl.)  (NdT. get B<back> = "obtenez", \ =
B<back>slash)
...
La valeur par défaut est "\034", la même que pour le SUBSEP en B<awk>.  Si vos
clefs contiennent des valeurs binaires, il se peut qu'il n'y ait aucune valeur
sûre pour "C<$;>".  (Mnémonique : la virgule (le séparateur d'indices) est un
demi-point-virgule. Ouais, je sais; c'est tiré par les cheveux, mais "C<$,>"
est déjà utilisé pour quelque chose de plus important.)
...
valeur B<awk>.  (Mnémonique : # est le signe des nombres -- numéros)
...
les formats.  (Mnémonique : % est le numéro de page pour B<nroff>.)
...
sortie en cours. Le défaut est 60.  (Mnémonique : = est formé de lignes
horizontales.)
...
(Mnémonique : lignes_sur_la_page - lignes_imprimées.)
...
Donc, après une recherche de motif dans $_, $& coïncide avec C<substr $_,
$-[0], $+[0] - $-[0]>. De la même manière, C<$>I<n> coïncide avec C<substr $_,
$-[>I<n>C<], $+[>I<n>C<] - $-[>I<n>C<]> si C<$-[>I<n>C<]> est défini, et $+
coïncide avec C<substr $_, $-[$#-], $+[$#-]>. On peut utiliser C<$#-> pour
trouver le dernier groupement dans la dernière recherche réussie. En
opposition avec C<$#+>, le nombre de groupements dans l'expression régulière.
Comparez avec C<@+>.
...
(Mnémonique : en poésie un "deux-points" fait partie de la ligne.)
...
[core dump].  (Mnémonique : similaire à B<sh> et B<ksh>.)
...
Ci-dessus, I<sans signification> est mis pour n'importe quoi :
...
initialiser C<$!> pour fixer C<errno> si, par exemple, vous voulez utiliser
C<"$!"> pour retourner le texte de l'erreur I<n>, ou si vous voulez fixer
la valeur de l'opérateur C<die()>.
(Mnémonique : Plantage !)
...
L'uid (id utilisateur) réel du processus.
(Mnémonique : l'uid d'I<OU> vous venez si vous exécutez setuid.)
...
(Mnémonique : l'uid I<VERS> lequel vous alliez, si vous exécutez setuid.)
Note : C<< $< >> and C<< $> >> peuvent être inversés seulement sur les machines
supportant C<setreuid()>.
...
Vous pouvez à la fois changer le gid réel et le gid effectif en
même temps en utilisant POSIX::setgid().

(Mnémonique : les parenthèses sont utilisées pour I<GROUPER> les choses. Le
gid réel est le groupe que vous laissez à votre I<GAUCHE>, si vous utilisez setgid.)
(NdT: I<GAUCHE> = parenthèse gauche)
...
setgroups() vide il faut utiliser : C< $) = "5 5" >.

Vous pouvez à la fois changer le gid réel et le gid effectif en
même temps en utilisant POSIX::setgid() (utilise seulement un unique
argument numérique).

(Mnémonique : les parenthèses sont utilisées pour I<GROUPER> les choses. Le
gid effectif est le groupe qui vous donne le bon I<DROIT> pour vous si vous
exécutez setgid.)  (NdT: I<DROIT> = parenthèse droite)

Note : C<< $< >>, C<< $> >>, C<$(> et C<$)> peuvent être positionnés
...
Sur certains systèmes, assigner une valeur à "C<$0>" modifie la zone
d'argument que le programme B<ps> voit. Plus utile pour indiquer l'état
courant du programme que pour cacher le programme en cours d'exécution.
(Mnémonique : comme en B<sh> et B<ksh>.)

Note pour les utilisateurs de BSD : fixer la valeur de C<$0> ne retire
pas complètement "perl" de la sortie de B<ps(1)>. Par exemple, donner la
valeur C<"foobar"> à C<$0> donne pour résultat C<$0>. C'est une
...
autres fichiers. Son usage est fortement déconseillé.
...
(Mnémonique : Cette version du Perl est-elle accrochée droite)
(Ndt : Les jeux de mots utilisés dans les mnémoniques anglais
...
La valeur des options de débuggage.
(Mnémonique : valeur de l'option B<-D>.)
...
ATTENTION : Cette variable est disponible pour utilisation interne
...
Par défaut, le dépassement de mémoire ne peut pas être capturé et entraîne
une erreur fatale. Cependant, s'il est compilé pour cela, Perl peut
utiliser le contenu de C<$^M> comme une réserve d'urgence après un die()
dû à un dépassement de mémoire. Supposons que votre Perl ait été compilé
avec l'option -DPERL_EMERGENCY_SBRK et utilise le malloc de Perl.
Dans ce cas

    $^M = 'a' x (1<<16);

allouera un buffer de 64K à utiliser en cas d'urgence. Voir le fichier
F<INSTALL> pour des informations sur la méthode permettant d'activer cette
option. Pour ne pas inciter à une utilisation intensive de cette fonction
avancée, il n'y a pas de noms longs L<English|English> pour cette variable.
...
est sujet à changement, mais est actuellement :
...
Note : Certains bits peuvent avoir un sens uniquement à la compilation ,
...
État en cours de l'interpréteur. Non défini si l'analyse du module/eval en
cours n'est pas finie (cela peut arriver dans $SIG{__DIE__} et
$SIG{__WARN__}).  Renvoie TRUE (vrai) si à l'intérieur d'un eval, FALSE (faux)
sinon.
...
Indique si le mode souillé est actif ou non (c'est-à-dire si le programme
a été lancé avec l'option B<-T> ou non). Vrai pour actif, faux pour inactif.
...
utilisez le motif C<"%vd"> de sprintf() :
...
(Mnémonique : comme l'option B<-w>.) Voir aussi L<warnings>.
...
Indicateur binaire [flag] global qui permet à Perl de faire des appels
système en utilisant les APIs natives qui gèrent les caractères
étendus, si elles sont disponibles. Option actuellement effective
uniquement sur la plate-forme Windows.

Peut aussi être activé depuis la ligne de commandes avec le
commutateur C<-C>.

La valeur initiale est typiquement C<0> pour assurer la compatibilité avec
les versions de Perl antérieures à la version 5.6, mais peut être mis à
C<1> automatiquement par Perl si le système fournit un réglage utilisateur
par défaut (par exemple C<$ENV{LC_CTYPE}>).

Le pragma C<bytes> a toujours priorité sur l'effet de ce flag dans la
portée lexicale courante. Voir L<bytes>.
...
Le descripteur de fichier spécial qui pointe le fichier de sortie ouvert courant
quand on fait de l'édition sur place avec B<I>. Utile quand vous devez faire
beaucoup d'insertions et que vous ne voulez pas modifier $_.
Voir L<perlrun> pour le commutateur B<I>.

=item @F

Le tableau @F contient les champs de chaque ligne lue quand le mode auto-découpage
(autosplit) est activé. Voir L<perlrun> pour le commutateur B<A>. Ce tableau est
spécifique au paquetage et doit être déclaré ou doit être invoqué avec un nom
pleinement qualifié s'il n'est pas dans le paquetage main quand on utilise
C<strict 'vars'>.
...
sous-programme, voyez L<perlfunc/require> pour une description de ces crochets),
ce crochet est inséré par défaut dans %INC à la place d'un nom de fichier.
Notez toutefois que le crochet a pu placer l'entrée dans %INC par lui-même
pour fournir quelques infos plus spécifiques.
...
Si votre système reconnaît la fonction sigaction(), alors les gestions de
signaux sont implémentées par cette fonction. Ce qui signifie que vous avez
une gestion des signaux fiable. Si votre système a l'option SA_RESTART, elle
est utilisée pour la gestion des signaux. Ce qui signifie que les appels
systèmes qui la supportent, continuent au lieu de s'interrompre quand ils
reçoivent un signal. Si vous voulez que vos appels systèmes soient interrompus
par un signal, faites quelque chose comme :
...
Certaines interruptions internes peuvent être mises en place avec %SIG. La
routine indiquée par C<$SIG{__WARN__}> est appelée quand un message
d'avertissement est sur le point d'être affiché. Le message est passé en
premier argument. La présence de l'indicateur __WARN__ supprime les
avertissements normaux sur STDERR. Vous pouvez vous en servir pour stocker les
avertissements dans une variable, ou pour les transformer en erreurs fatales,
comme ceci :
...
Note : C<__DIE__>/C<__WARN__> ont ceci de spécial, qu'ils peuvent
...
peut se produire si les fonctions C<open> ou C<close> ont été mal importées),
ou si le code Perl exécuté pendant l'évaluation échoue via die().
Dans ce cas $@ contient l'erreur de compilation, ou l'argument de C<die>
(qui interpolera C<$!> et C<$?>!). (Néanmoins, voir aussi L<Fatal>.)
...
Sous quelques systèmes d'exploitation, C<$^E> peut contenir un indicateur
d'erreur plus verbeux, tel que, dans ce cas : "Le tiroir du CDROM n'est pas
fermé."  Sur les systèmes qui ne supportent pas de messages d'erreur étendus,
C<$^E> contient la même chose que C<$!>.
...
lettres, des chiffres, des traits-de-soulignement, ou la séquence spéciale
C<::> ou encore C<'>.  Dans ce cas, la partie placée avant le dernier C<::> ou
C<'> est interprétée comme un I<qualificateur de paquetage>; voir L<perlmod>.

Les noms de variables en Perl peuvent être aussi une séquence de chiffres ou
un signe de ponctuation unique ou un caractère du contrôle. Ces noms sont tous
réservés par Perl pour des usages spéciaux ; par exemple, les noms 'tout en
chiffres' sont utilisés pour contenir les données capturées par les
références-arrières d'une expression rationnelle après une recherche de motif
réussie. Perl a une syntaxe spéciale pour les noms 'caractère de contrôle
seul' : il comprend C<^X> (circonflexe C<X>) comme signifiant le caractère
contrôle-C<X>. Par exemple, la notation C<$^W> (dollar circonflexe C<W>) est
la variable scalaire dont le nom est le seul caractère contrôle-C<W>. C'est
mieux que de taper un contrôle-C<W> directement dans votre programme.

Finalement, ce qui est nouveau dans Perl 5.6, les noms de variables peuvent
être des chaînes alphanumériques qui commencent par des caractères de contrôle
(ou mieux encore, un circonflexe). Ces variables doivent être écrites sous la
forme C<${^Foo}>; les accolades ne sont pas facultatives. C<${^Foo}> dénote la
variable scalaire dont le nom est un contrôle-C<F> suivi par deux C<o>. Ces
variables sont réservées par Perl pour de futurs usages spéciaux, à
l'exception de ceux qui commencent par C<^_> (contrôle-soulignement ou
circonflexe-soulignement).  Aucun nom du type contrôle-caractère qui commence
par C<^_> n'acquerra de signification spéciale dans toute future version de
Perl ; de tels noms peuvent donc être utilisés sans risque dans les
programmes.  Toutefois C<$^_> lui-même, I<est> réservé.

Les identificateurs Perl qui commencent par des chiffres, des caractères de
contrôle, ou des caractères de ponctuation sont exemptés des effets de la
déclaration dans un C<paquetage> et sont toujours forcés à être dans le
paquetage C<main>.  Quelques autres noms en sont aussi exemptés:
...
En particulier, les nouvelles variables spéciales C<${^_XYZ}> sont toujours
associées au paquetage C<main>, indépendamment de toute déclaration de
paquetage dans la portée.

=head1 BUGS

Dû à un accident fâcheux dans l'implémentation de Perl, C<use English>
impose une pénalité de performance considérable sur toutes les recherches
de motif dans un programme, indépendamment de l'endroit où elles se
trouvent dans la portée de C<use English>. Pour cette raison, l'utilisation
de C<use English> est fortement déconseillée dans les bibliothèques.
Voir la documentation du module Devel::SawAmpersand du CPAN
...
Fabien Martinet <ho.fmartinet@cma-cgm.com> (version de perl 5.00502)

=head2 Relecture

Jean-Louis Morel <jl_morel@bribes.org>     (mise à jour version de perl 5.8.0)

...
coûteux : le contrat spécifie que la garantie disparaît si vous ouvrez le
...
Perl : vous pouvez le déréférencer non seulement par une chaîne ou un nombre
...
qu'objet. Rappelez-vous : être un objet ne signifie rien de plus que pouvoir
...
Puisque nous y sommes, rendons notre constructeur un peu plus flexible. Plutôt
que d'en faire juste une méthode de classe, modifions le pour qu'il soit
utilisable comme méthode de classe I<ou> comme méthode d'objet. De cette
manière, nous pourrons écrire :

    $me  = Person->new();
    $him = $me->new();

Pour obtenir ce résultat, la seule chose à faire est de tester si l'argument
qui nous est passé est une référence ou non. Si oui, c'est que nous sommes
appelé comme méthode d'objet et que nous devons donc extraire le paquetage (la
classe) en utilisant la fonction ref(). Sinon, nous utilisons la chaîne passée
en argument comme le nom du paquetage pour bénir (blessing) notre référence.
...
références circulaires comme celle-ci :
...
que donner l'accès en lecture et/ou écriture à un champ particulier :
...
ligne suivante au début de votre fichier :
...
le constructeur $Person::new() de la manière suivante : 
...
décrémenter $Census lorsqu'un objet Person est détruit. Voici ce destructeur :
...
données de classe. Un bonne règle est : I<vous ne devriez jamais référencer
...
utiliser les deux appels suivants :
...
n'effectue un 'exec' ou qu'il meurt sur un signal non capté. Par exemple :
...
parties de code. Voici un exemple de pod décrivant notre interface :
...
pouvez écrire un programme comme celui-ci :
...
debug(), mais en hérite d'ailleurs ? Il aurait été préférable de dire :
...
Comme ça, la recherche s'effectue dans tous les classes du tableau @ISA. Cela
n'a un sens I<que> lors de l'appel d'une méthode. N'essayez pas d'utiliser
SUPER dans un autre contexte parce qu'elle n'existe que dans le cas d'appel à
...
notre classe de test sera dérivée de Employee. En voici une :
...
pouvez utiliser le module Data::Dumper disponible au CPAN :
...
sait rien des Employee (elle ne le doit pas : une bonne conception orientée
...
Bon, au risque d'ennuyer les gourous OO et de perturber les débutants, il est
temps d'avouer que le système objet de Perl propose la notion très
controversée d'héritage multiple (ou HM pour faire court). Cela signifie qu'au
lieu d'hériter d'une classe qui elle-même peut hériter d'une autre classe et
ainsi de suite, vous pouvez hériter directement de plusieurs classes
parentes. Il est vrai que le HM peut rendre les choses confuses, même si en
Perl cela l'est un peu moins qu'avec des langages OO douteux comme le C++.

La manière dont cela fonctionne est vraiment très simple : il suffit de mettre
plus d'un nom de paquetage dans le tableau @ISA. Lorsque Perl cherche une
méthode pour votre objet, il regarde dans chacun de ces paquetages dans
l'ordre. C'est une recherche récursive en profondeur d'abord. Supposons un
ensemble de tableaux @ISA comme :
...
Comment allons-nous trouver la méthode find() (ou une méthode new()) ? Puisque 
la recherche s'effectue en profondeur d'abord, les classes seront explorées
dans l'ordre suivant : Third, First, Alpha, Second et enfin Beta.
...
l'HM. C'est pourquoi notre objet Person I<contient> un objet Fullname. Cela ne
veut pas dire qu'il en I<est> un.

Par contre, il y a un domaine où l'HM de Perl est très répandu : lorsqu'une
...
un élément supplémentaire à la fin de @ISA : la classe UNIVERSAL. Dans la
...
parcourir vous-même la hiérarchie d'héritage :
...
VERSION, cela arrive quand vous dites :

    use Some_Module 3.0;

Si vous voulez ajouter un contrôle de version à votre classe Person, ajoutez
juste dans Person.pm :

    our $VERSION = '1.1';

Vous pourrez donc dire dans Employee.pm :
...
l'exécution de cette méthode. Ou presque rien : vous I<pourriez> utiliser le
...
de _CENSUS : c'est un indicateur qui signifie "à usage interne seulement".

Voici à quoi ressemblent le code d'initialisation et le constructeur de la
classe si nous suivons cette approche :
...
classes. Deux d'entre elles méritent notre attention : Class::Struct et
...
Le point crucial est cet appel :
...
redéfinissent les fonctions de base avec des structures : File::stat,
...
Lorsque ce travail est inclus comme un élément de la distribution standard de
Perl ou comme une partie de sa documentation complète sous forme imprimée ou
autrement, il ne peut être distribué que dans les limites fixées par la Perl's
Artistic License. Toute distribution de ce fichier ou de ses dérivés I<hors>
de cet ensemble nécessite un accord particulier avec le titulaire des droits.

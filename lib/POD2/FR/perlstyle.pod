...
Couper Les lignes trop longues après un opérateur (sauf «E<nbsp>andE<nbsp>» et
«E<nbsp>orE<nbsp>»).
...
Évitez d'utiliser grep() (ou map()) ou des «E<nbsp>backticksE<nbsp>» (`) dans
un contexte vide, c'est-à-dire, quand vous ne récupérez pas les valeurs
qu'elles retournent. Ces fonctions retournent toujours des valeurs alors,
utilisez-les. Sinon, utilisez une boucle foreach() ou l'appel system() à la
place.
...
Bien que les petits identificateurs comme $gotit soient compréhensibles, les
longs sont plus lisibles si les mots sont séparés par des underscores. Il est
plus facile de lire $un_nom_de_variable que $UnNomDeVariable, surtout pour
ceux qui ne parlent pas très bien la langue dans laquelle le programme a été
écrit. C'est aussi une règle simple qui marche aussi avec UN_NOM_DE_VARIABLE.
...
elles sont en minuscule. ExE<nbsp>: $obj-E<gt>as_string().
...
Utilisez les nouveaux opérateurs «E<nbsp>andE<nbsp>» et «E<nbsp>orE<nbsp>»
pour éviter d'avoir à mettre trop de parenthèses dans les listes d'opérations
et pour réduire l'utilisation des opérateurs tels C<&&> et C<||>. Appelez vos
routines comme s'il s'agissait de fonctions ou d'opérateurs de listes pour
éviter le surplus de parenthèses et de «E<nbsp>&E<nbsp>».

=item *

Utilisez des C<here> (opérateur C<E<lt>E<lt>>) plutôt que des instructions
C<print()> répétés.
...
Vérifiez toujours la valeur retournée par un appel système. Les meilleurs
messages d'erreur sont ceux qui sont dirigés sur STDERR et qui S<fournissent
:> le programme en cause, l'appel système qui a échoué avec ses arguments et
(TRÈS IMPORTANT) le message d'erreur système indiquant la cause de
l'échec. Voici un exemple simple, mais S<suffisant :>
...
Mise à jour S<v5.8.5 :> Paul Gaborit <Paul.Gaborit @ enstimac.fr>

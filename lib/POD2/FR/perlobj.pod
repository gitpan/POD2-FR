...
=head2 Un objet est simplement une référence
...
=head2 Une classe est simplement un paquetage
...
Il existe un tableau spécial appelé @ISA à l'intérieur de chaque paquetage,
qui dit où trouver une méthode si on ne la trouve pas dans le paquetage
courant. C'est de cette façon que Perl implémente l'héritage. Chaque élément
du tableau @ISA est juste le nom d'un autre paquetage qui s'avère être un
paquetage de classe. Les classes sont fouillées (en profondeur tout d'abord) à
la recherche des méthodes manquantes dans l'ordre où elles apparaissent dans
@ISA. Les classes accessibles à travers @ISA sont les classes de base de la
classe courante.
...
=head2 Une méthode est simplement un sous-programme
...
Il est important de noter que C<SUPER> se réfèrent à la (aux) superclasse(s)
du I<paquetage courant> et non à la (aux) superclasse(s) de l'objet lui-même. De
plus, la pseudo classe C<SUPER> peut être utilisée comme modificateur d'un nom
de méthode mais pas aux autres endroits où un nom de classe est
utilisé. S<Exemple :>
...
Vous pouvez aussi appeler C<UNIVERSAL::isa> comme une subroutine avec deux
arguments. Le premier n'étant pas nécessairement un objet ni même une
référence. Cela vous permet de vérifier ce vers quoi pointe une référence ou
si c'est une référence du bon type. S<Exemple :>

    if(UNIVERSAL::isa($ref, 'ARRAY')) {
        #...
    }

Pour déterminer si une référence est bien un objet consacré, vous pouvez
S<écrire :>
...
arguments. Elle retourne toujours I<undef> si son premier argument n'est pas
un objet ou le nom d'une classe. Voici donc un autre moyen de vérifier qu'une
référence est un objet S<consacré :>

    print "C'est encore un objet\n" if UNIVERSAL::can($val, 'can');

Vous pouvez aussi utiliser la fonction C<blessed> de S<C<Scalar::Util> :>

    use Scalar::Util 'blessed';

    my $blessing = blessed $suspected_object;

C<blessed> retourne le nom du paquetage ayant consacré son argument ou C<undef>.
...
=head2 Ramasse-miettes à deux phases
...
Roland Trique <F<roland.trique@free.fr>>. Mise à jour S<5.8.5 :> Paul Gaborit

...
Ce document à pour but de donner une vue d'ensemble du langage de
...
doubles. Une chaîne de caractère encadrée par des apostrophes s'affiche telle
...
Les scalaires peuvent être indifféremment des chaînes de caractères, des
entiers, des nombres en virgules flottante, des références (voir plus loin),
plus quelques valeurs spéciales. Perl procédera automatiquement aux
conversions nécessaires lorsque c'est cohérent (par exemple pour transformer
en nombre une chaîne de caractère représentant un nombre). Il est inutile de
déclarer au préalable le type des variables.
